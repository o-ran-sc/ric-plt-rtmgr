...
* In case of non-Docker build: golang 11.1 at least, go-swagger (https://github.com/go-swagger/go-swagger), glide (https://github.com/Masterminds/glide)
...
### Compiling code
Alternatively, if you wish to test the binary (without Kubernetes) you can compile with `go build cmd/rtmgr.go` while standing in the project root directory. Make sure you issue command `glide install --strip-vendor` to fetch all dependencies.
