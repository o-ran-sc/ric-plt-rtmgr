...
func (r *Rpe) generatePlatformRoutes(e2TermEp []rtmgr.Endpoint, subManEp *rtmgr.Endpoint, e2ManEp *rtmgr.Endpoint, a1mediatorEp *rtmgr.Endpoint, routeTable *rtmgr.RouteTable) {
	...
	if len(e2TermEp) > 0 {
		...
		for _, e2TermEps := range e2TermEp {
			r.addRoute("RIC_E2_RESET_REQ_FROM_E2", &e2TermEps, e2ManEp, routeTable, -1, "")
		}

		r.addRoute_rx_list("RIC_E2_RESET_RESP_FROM_RIC", e2ManEp, e2TermEp, routeTable, -1, "")
	}
